...
global function ServerToClient_ShowHealthRUIWithLosCheck
...
struct
{
	...
	float sonicBlastScanDurationMultiplierWithSpitCharge
...
void function MpAbilitySonicBlast_Init()
{
	...
	file.sonicBlastScanDurationMultiplierWithSpitCharge = GetSonicBlastScanDurationMultiplierWithSpitCharge()
...
{
	thread ServerToClient_ShowHealthRUI_Thread( owner, victim, duration, 0 )
}

void function ServerToClient_ShowHealthRUIWithLosCheck( entity owner, entity victim, float duration, float losDuration )
{
	thread ServerToClient_ShowHealthRUI_Thread( owner, victim, duration, losDuration )
}

void function ServerToClient_ShowHealthRUI_Thread( entity owner, entity victim, float noLosDuration, float losRequiredDuration )
{
	Assert ( IsNewThread(), "Must be threaded off." )
	...
	if ( noLosDuration == 0 && losRequiredDuration == 0 )
		return
	...
	float endTime = Time() + max( losRequiredDuration, noLosDuration)
	float noLosEndTime = Time() + noLosDuration
	float losRequiredEndTime = Time() + losRequiredDuration

	PassByReferenceBool noLosRequested
	PassByReferenceBool losRequiredRequested

	var rui = null
	if ( noLosDuration > 0 )
	{
		noLosRequested.value = true
		rui = ReconScan_ShowHudForTarget( owner, victim, eShowHealthbarMode.ALWAYS )
	}
	if ( losRequiredDuration > 0 )
	{
		losRequiredRequested.value = true
		rui = ReconScan_ShowHudForTarget( owner, victim, eShowHealthbarMode.REQUIRE_LOS )
	}
	...
	OnThreadEnd(
		function() : ( owner, victim, noLosRequested, losRequiredRequested )
		{
			if ( noLosRequested.value )
			{
				ReconScan_RemoveHudForTarget( owner, victim, eShowHealthbarMode.ALWAYS )
			}
			if ( losRequiredRequested.value )
			{
				ReconScan_RemoveHudForTarget( owner, victim, eShowHealthbarMode.REQUIRE_LOS )
			}
		}
	)
	...
	while ( Time() < endTime )
	{
		if ( noLosRequested.value && Time() >= noLosEndTime )
		{
			ReconScan_RemoveHudForTarget( owner, victim, eShowHealthbarMode.ALWAYS )
			noLosRequested.value = false
		}

		if ( losRequiredRequested.value && Time() >= losRequiredEndTime )
		{
			ReconScan_RemoveHudForTarget( owner, victim, eShowHealthbarMode.REQUIRE_LOS )
			losRequiredRequested.value = false
		}
...
float function GetSonicBlast_Silence_Duration_Base( entity player )
{
	...
	if( !player.IsPlayer() )
		...
		{
			result *= file.sonicBlastScanDurationMultiplierWithSpitCharge
		}
...
float function GetSonicBlastScanDurationMultiplierWithSpitCharge()
{
	return GetCurrentPlaylistVarFloat( "seer_tac_upgrade_3_mutiplier", 0.5 )
}
